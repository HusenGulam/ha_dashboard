...
export default function CustomizedTables({Base_style}) {
      ...
      <Table sx={{ minWidth: 400 }} aria-label="customized table">
        <TableHead  >
          <TableRow>
            ...
            <StyledTableCell colSpan={3} align="center">Tugatgan vaqti</StyledTableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {ResultsData?.map((row) => (
            <StyledTableRow key={row.name} style={{backgroundColor:row.name === 'Dostonbek Abdumuxtorov' ? "orange" : ''}}>
